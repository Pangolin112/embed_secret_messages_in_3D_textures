...
from utils.scannetpp_dataloader import (
    ...
    load_depth,
    rescale_and_crop,
    rescale_and_crop_semantic,
)


def scannetpp_large_scene_datasaver_semantic_chunk():
    """
    ...
    scene_list = ['e9ac2fc517'] # '0cf2e9402d' '49a82360aa' 'fb5a96b1a2' e9ac2fc517, e3ecd49e2b

    # import argparse
    # parser = argparse.ArgumentParser()
    # parser.add_argument("--input_dir", type=str, help="original dataset directory", default="/media/qianru/12T_Data/Data/ScanNetpp/")
    ...
    ScanNetpp_path_output = '/home/qianru/Projects/TUM/TUM_4/GR/ScanNetpp_512_Semantic/'
    ...
    for scene_id in tqdm(scene_list):
        ...
        if len(train_frames) < 1000:
            continue  # skip scenes with less than 1000 frames
        
        print(f"\n Processing scene {scene_id} with {len(train_frames)} frames... \n")
        ...
        # load semantic maps
        semantic_root = os.path.join(ScanNetpp_path, "2d_semantic_consistent_labels", "viz_obj_ids", scene_id)
        ...
        for chunk in chunk_loop:
            ...
            # load semantic maps
            semantics = [load_image(os.path.join(semantic_root, frame["file_path"] + ".png")) for frame in chunk]
            semantics = torch.stack(semantics)  # (N, 3, h, w)

            # center crop image, depth, intrinsics to (1024, 1024)
            w = h = render_size
            images, K_croped, depth, semantics = rescale_and_crop_semantic(images, K, (h, w), depth, semantics)
            ...
            os.makedirs(os.path.join(out_dir, "semantics"), exist_ok=True)
            ...
            for i, frame in enumerate(chunk):
                ...
                semantic_path = f"./semantics/{base}.png"
                ...
                # save semantic map
                semantic = (semantics[i].cpu().permute(1, 2, 0).numpy() * 255).astype(np.uint8)
                Image.fromarray(semantic).save(os.path.join(out_dir, semantic_path))

                # collect metadata
                out_transforms["frames"].append({
                    "file_path": img_path,
                    "depth_file_path": depth_path,
                    "semantic_file_path": semantic_path,
        ...
        print(f"\n Saved {len(train_frames)} frames to {out_dir} \n")
