  ...
  if (index < channels * tensorSize)
    {
      ...
      unsigned int bin = uv < (nBins - 1) ? uv : (nBins - 1);
  ...
  if (index < channels * tensorSize)
    {
      ...
      unsigned int uv = (unsigned int)(value);
      unsigned int binIndex = uv < (nBins - 1) ? uv : (nBins - 1);
...
at::Tensor computeHistogram(at::Tensor const &t, unsigned int numBins)
{
  ...
  auto mins = std::get<0>(unsqueezed.min(1, true)).cuda();
  auto maxs = std::get<0>(unsqueezed.max(1, true)).cuda();
...
void matchHistogram(at::Tensor &featureMaps, at::Tensor &targetHistogram)
{
  ...
  // add this code to compute the loss on images larger than 512*512 //
  if (randomIndices[n*c].numel() != n*c)
    randomIndices[n*c] = torch::randperm(n*c, torch::TensorOptions().dtype(at::kLong)).cuda();
  // add this code to compute the loss on images larger than 512*512 //
  ...
  buildSortedLinkmap<<<(c*n) / THREAD_COUNT + 1, THREAD_COUNT>>>(featureMaps.data<float>(), linkMap, featuresHistogram.data<float>(), localIndexes, randomIndices[featureMaps.numel()].data<long>(), mins.data<float>(), maxs.data<float>(), c, n, nBins);
