...
cpp = torch.utils.cpp_extension.load(name="histogram_cpp", sources=["./utils/histogram/histogram.cpp", "./utils/histogram/histogram.cu"])
...
class StyleTransfer(torch.nn.Module):
    ...
    def optimise(self, canvas = None):
        ...
        for i in loop:
            ...
            loop.set_description(f'{self.histogram}, {canvas.shape[2]}, {i}, {loss.item()}')
            loss.backward()
            optimizer.step()
        canvas.clamp(0, 1)
        return canvas.data
    
class histogram_loss():
    def __init__(self, histogram=True):
        self.histogram = histogram
        self.model = StyleTransfer().cuda()
        self.model.histogram = histogram

    def __call__(self, x, y):
        self.style_transfer.setStyle(transform(y))
        return self.style_transfer.computeLoss(transform(x))
    
    def optimise(self, canvas=None):
        return self.style_transfer.optimise(canvas)