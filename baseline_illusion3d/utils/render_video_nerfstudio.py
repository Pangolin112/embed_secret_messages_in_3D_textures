...
from utils.pytorch3d_uv_utils import (
    ...
    data_prepare_scannetpp_video_render,
...
def render_video_nerfstudio():
    ...
    output_dir_checkpoint = 'outputs/baseline_sds/2025-09-02_22-33-40_A TUM logo image_A Japanese style living room_num_steps_4000_seed_99' # checkpoint directory
    ...
    render_camera_path = 'data/ScanNetpp/49a82360aa/dslr/camera_paths/2025-06-24-19-05-05.json'
    # render_camera_path = 'data/ScanNetpp/e9ac2fc517/dslr/camera_paths/2025-07-22-14-36-18.json'
    # render_camera_path = 'data/ScanNetpp/fb5a96b1a2/dslr/camera_paths/2025-06-17-11-41-38.json'
    
    with open(render_camera_path, "r") as f:
        render_camera_data = json.load(f)
    transform_data_path = f'data/ScanNetpp/{scene_name}/dslr/camera_paths/dataparser_transforms.json'
    with open(transform_data_path, "r") as f:
    ...
    for scene_id in tqdm(scene_list):
        ...
        video_c2w = np.array([np.array(frame['camera_to_world'], dtype=np.float32).reshape([4, 4]) for frame in render_camera_data['camera_path']])

        T, s = transform_data['transform'], transform_data['scale']
        T.append([0, 0, 0, 1.0])
        ...
        video_c2w[:, :3, 3] /= s
        video_c2w = np.array([T_inv @ c2w for c2w in video_c2w], dtype=np.float32)

        # video_c2w = video_c2w[:20]
    ...
    uv_coords_list = data_prepare_scannetpp_video_render(video_cameras, device, render_size, faces_per_pixel, new_mesh)
    ...
    print(f"saved video {output_video_file}")
