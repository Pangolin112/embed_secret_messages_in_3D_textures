...
def vis_cams_and_mesh(c2w, K, height, width, mesh_path):
    device = "cuda:0"
...
def render_images_from_mesh(images, depth, c2w, K, height, width, mesh_path):
    device = "cuda:0"
    c2w = torch.from_numpy(c2w).to(device)
    K = K.to(device)
...
def main():
    ...
    parser.add_argument("--input_dir", type=str, help="original dataset directory", default="/cluster/eriador/lhoellein/scannetpp/")
    ...
    for scene_id in tqdm(scene_list):
        ...
        # vis_cams_and_mesh(train_c2w, K, h, w, mesh_path)

        # sample renderings
        render_images_from_mesh(images, depth, train_c2w, K, h, w, mesh_path)
