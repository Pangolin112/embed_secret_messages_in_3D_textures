...
# -----------------------------------------------------------------------------
# DSSIM helper functions (Gaussian window, SSIM→DSSIM)
# -----------------------------------------------------------------------------
...
def Tex2Tex_GaussTrap():
    ...
    # output paths
    output_path = './outputs/Tex2Tex_GaussTrap'
    os.makedirs(output_path, exist_ok=True)

    work_dir = output_path + '/' + str(datetime.now().strftime("%Y-%m-%d_%H-%M-%S")) + f"_epoch_{num_epochs}_lr_{lr}_use_mse_{use_mse_loss}_l1_weight_{l1_weight}_attack_{num_attack_iters}_stabilize_{num_stabilize_iters}_normal_{num_normal_iters}"
    ...
    images_ves = torch.stack(images_ves, dim=0)  # (N, 3, H, W), [0, 1]
    ...
    # update dateset
    images_update[secret_view_idx] = edited_image_secret.detach() # [0, 1]

    save_ves_results((edited_image_secret - 0.5) * 2, uv_coords_ves_list, model, device, 'initial_selection', rgb_path_stage_2)
    ...
    for epoch in loop_epochs:
        ...
        if (epoch + 1) % epoch_period == 0 or epoch == num_epochs - 1 or epoch == 0:
            ...
            image_save_ves = torch.cat([depth_tensor_ves, rgb_ves / 2 + 0.5, image_ves])
            save_image((image_save_ves).clamp(0, 1), rgb_path_stage_2 + f'/ves_{epoch + 1}_image.png')
            
            image_save = torch.cat([depth_tensor, rgb / 2 + 0.5, image])
