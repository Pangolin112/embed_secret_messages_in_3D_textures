...
def PTDiffusion_2d_to_3d():
    ...
    output_path = './outputs/PTDiffusion_2d_to_3d'
    ...
    # origianl PTD
    # latents_PT = torch.randn_like(ref_latent)
    ...
    for i in loop_stage_2:
        ...
        if i % int(Niter_stage_2 / ref_steps) == 0: # only do DDIM step for ref_steps steps
            ...
            ref_a_prev, ref_pred_x0, ref_e = p_sample_ddim( # last update ration, last step prediction, predicted noise
                ref_steps, ref_latent, uncond, ts_tensor_ref, t_ref, t_ref_last,
                scheduler, unet, return_all=True
            )
            # direct
            ref_latent = ref_a_prev.sqrt() * ref_pred_x0 + (1. - ref_a_prev).sqrt() * ref_e # noisy ref latents
        ...
        for j in np.random.permutation(num_views).tolist()[:num_views_per_iteration]:
            ...
            for model in particles:
                ...
                # do DDIM step to the current latents
                # update chosen t and last t
                ...
                ts_tensor = torch.full((ref_latent.size(0),), chosen_t, device=ref_latent.device, dtype=torch.long)

                dec_a_prev, dec_pred_x0, dec_e = p_sample_ddim_depth( # last update ration, last step prediction, predicted noise
                    Niter_stage_2, latents, cond, ts_tensor, t, t_last,
                    scheduler, unet,
                    controlnet, conditioning_scale, controlnet_cond_input, text_embeddings_2,
                    unconditional_guidance_scale=guidance_scale,
                    unconditional_conditioning=uncond,
                    return_all=True
                )
                # reconstruct
                latents_rec = dec_a_prev.sqrt() * dec_pred_x0 + (1 - dec_a_prev).sqrt() * dec_e # noisy latents
                latents_rec = latents_rec.to(dtype_half)
                ...
                #     noisy_latents = phase_substitute(ref_latent, noisy_latents_original, blending_ratio)
                # else:
                #     # add noise after phase transfer
                #     latents_transferred = phase_substitute(ref_latent, latents, blending_ratio)
                #     noisy_latents = scheduler.add_noise(latents_transferred, noise, t)

                latents_transferred = phase_substitute(ref_latent, latents_rec, blending_ratio)

                latents_transferred_clean = phase_substitute(ref_pred_x0, dec_pred_x0, blending_ratio)

                # original PTD
                # latents_PT = latents_transferred
                # latents = latents_rec
                ...
                loss_sds = 0.5 * F.mse_loss(noisy_latents - latents, latents_transferred.detach() - latents, reduction="mean")

                # loss_sds = 0.5 * F.mse_loss(latents, latents_transferred.detach(), reduction="mean")

                # loss_sds = 0.5 * F.mse_loss(latents, latents_PT.detach(), reduction="mean")
            ...
            # Print the losses
            # loop_stage_2.set_description(f"stage: {stage}, total_loss = %.6f, loss_phi = {loss_phi}" % sum_loss)
            loop_stage_2.set_description(f"stage: {stage}, total_loss = %.6f" % sum_loss)
            # original PTD
            # loop_stage_2.set_description(f"stage: {stage}")

            # save results
            if i % log_step == 0:
                tmp_latents = 1 / vae.config.scaling_factor * latents.clone().detach()
                # tmp_latents_PT = 1 / vae.config.scaling_factor * latents_PT.clone().detach()
                pred_latents = 1 / vae.config.scaling_factor * dec_pred_x0.to(dtype_half).clone().detach()
                # pred_latents = scheduler.step(dec_e, t, latents_rec).pred_original_sample.to(dtype_half).clone().detach()  / vae.config.scaling_factor
                pred_latents_transfer = 1 / vae.config.scaling_factor * latents_transferred_clean.clone().detach()
                # pred_latents_transfer = scheduler.step(dec_e, t, latents_transferred).pred_original_sample.to(dtype_half).clone().detach() / vae.config.scaling_factor
                ref_latent_current = 1 / vae.config.scaling_factor * ref_latent.clone().detach()
                with torch.no_grad():
                    image_ = vae.decode(tmp_latents).sample.to(dtype_half)
                    # image_PT = vae.decode(tmp_latents_PT).sample.to(dtype_half)
                    image_x0 = vae.decode(pred_latents).sample.to(dtype_half)
                    image_x0_transfer = vae.decode(pred_latents_transfer).sample.to(dtype_half)
                    image_ref = vae.decode(ref_latent_current).sample.to(dtype_half)
                    image = torch.cat((image_, image_x0, image_x0_transfer, depth_tensor, image_ref), dim=2)
                    # image = torch.cat((image_, depth_tensor, image_ref), dim=2)
                ...
                # save_image((image_PT / 2 + 0.5).clamp(0, 1), f'{rgb_path_stage_2}/rgb_PT_{i}_t{t.item()}.png')
                save_image((image_x0 / 2 + 0.5).clamp(0, 1), f'{rgb_path_stage_2}/predicted_x0_{i}_t{t.item()}.png')
                save_image((image_x0_transfer / 2 + 0.5).clamp(0, 1), f'{rgb_path_stage_2}/predicted_x0_transfer_{i}_t{t.item()}.png')
