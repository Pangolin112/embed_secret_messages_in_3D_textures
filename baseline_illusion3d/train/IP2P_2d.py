...
def IP2P_2d():
    ...
    prompt = 'make it a japanese style living room'
    ...
    ptd = PTD(dtype, ref_img_path, conditioning_scale, prompt)

    ip2p = InstructPix2Pix(device)
    # ip2p configs
    text_embeddings = torch.cat([ptd.cond, ptd.uncond, ptd.uncond])
    guidance_scale = 7.5
    image_guidance_scale = 1.5
    diffusion_steps = 20
    lower_bound = 0.98 #0.02
    upper_bound = 0.99

    output_path = './outputs/IP2P_2d'
    os.makedirs(output_path, exist_ok=True)

    work_dir = output_path + '/' + str(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    ...
    for i in loop:
        ...
        # edited_image, depth_tensor = ptd.edit_image_depth(image, depth)

        image_save = torch.cat([image, edited_image], dim=0)
