...
# main function
def Instruct_Tex2Tex_2_secrets():
    ######################################################
    # 1. Set configs                              
    ######################################################
    ...
    # secret view index
    secret_view_name = config['secret_view_name']
    secret_view_name_2 = config['secret_view_name_2']
    ...
    for scene_id in tqdm(scene_list):
        ...
        secret_view_idx_2 = train_frames.index(
            next(frame for frame in train_frames if frame["file_path"] == "./images/" + secret_view_name_2)
        )
        print(f"Secret view index: {secret_view_idx}, secret view image name: {secret_view_name}")
        ...
        rgb_root = os.path.join(ScanNetpp_path, scene_id)
        ...
        depth_root = os.path.join(ScanNetpp_path, scene_id)
        depths = [load_depth(os.path.join(depth_root, frame["depth_file_path"].replace(".JPG", ".png"))) for frame in train_frames]
    ...
    losses = {
        ...
        "secret_2": {"weight": secret_weight, "values": []},  # second secret view loss
    }
    ...
    # prepare second secret view's data
    uv_coords_secret_2 = uv_coords_list[secret_view_idx_2].to(device)
    zero_mask_secret_2 = (uv_coords_secret_2[..., 0] == 0) & (uv_coords_secret_2[..., 1] == 0)
    depth_tensor_secret_2 = depth_tensor_list[secret_view_idx_2].to(device)
    depth_secret_2 = depths[secret_view_idx_2].to(device).unsqueeze(0).to(dtype) # B, H, W, [0, 1], meter
    image_secret_2 = images[secret_view_idx_2].to(device).unsqueeze(0).to(dtype) # B, 3, H, W, [0, 1]
    ...
    for i in loop_stage_2:
        ...
        for j in np.random.permutation(num_views).tolist()[:num_views_per_iteration]:
            ...
            for model in particles:
                ...
                # second secret view
                rgb_secret_2 = get_rgb(uv_coords_secret_2, zero_mask_secret_2, model, device)
                rgb_secret_2 = rgb_secret_2.to(dtype)
                ...
                if i % edit_rate == 0 or i == Niter_stage_2 - 1:
                    ...
                    # second secret view
                    edited_image_secret_2, depth_tensor_secret_2 = ip2p_ptd.edit_image_depth(
                        image=rgb_secret_2 / 2 + 0.5, # input should be B, 3, H, W, in [0, 1]
                        image_cond=image_secret_2,
                        secret_idx=1,
                        depth=depth_secret_2,
                        lower_bound=lower_bound,
                        upper_bound=upper_bound
                    ) 
                    # update dateset
                    images_update[secret_view_idx_2] = edited_image_secret_2
                ...
                # get second secret target
                target_secret_2 = (images_update[secret_view_idx_2].to(device).unsqueeze(0).permute(0, 2, 3, 1) - 0.5) * 2
                expanded_zero_mask_secret_2 = zero_mask_secret_2.unsqueeze(-1).expand_as(target_secret_2)
                target_secret_2 = target_secret_2.masked_fill(expanded_zero_mask_secret_2, -1)
                target_secret_2 = target_secret_2.permute(0, 3, 1, 2).to(device)
                target_secret_2 = target_secret_2.clamp(-1, 1)
                target_secret_2 = target_secret_2.to(dtype)
                ...
                if i % secret_update_rate == 0: # every secret_update_rate iters, update once for a secret view, every iter, update once for a non-secret view.
                    ...
                    loss_secret_2 = 0.5 * F.mse_loss(rgb_secret, target_secret_2, reduction="mean")
                else:
                    loss_secret = 0.0
                    loss_secret_2 = 0.0

                loss["secret"] += loss_secret / num_views_per_iteration
                loss["secret_2"] += loss_secret_2 / num_views_per_iteration
            ...
            if i % log_step == 0:
                ...
                image_save_secret_2 = torch.cat([depth_tensor_secret_2, rgb_secret_2 / 2 + 0.5, edited_image_secret_2, image_secret_2])
                save_image((image_save_secret_2).clamp(0, 1), rgb_path_stage_2 + f'/secret_{i}_image_2.png')
    ...
    image_save_secret_2 = torch.cat([depth_tensor_secret_2, rgb_secret_2 / 2 + 0.5, edited_image_secret_2, image_secret_2])
    save_image((image_save_secret_2).clamp(0, 1), rgb_path_stage_2 + f'/secret_{i + 1}_image_2.png')
